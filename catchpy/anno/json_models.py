...
class AnnoJS(object):
    ...
    def convert_from_anno(cls, anno):
        ...
        try:
            ...
            msg = "anno({}) missing platform property: {}".format(anno.anno_id, str(e))
            logger.error(msg, exc_info=True)
            raise AnnotatorJSError(msg)
    ...
    def convert_target(cls, anno):
        ...
        if anno.target_type == RESOURCE_TYPE_LIST:
            ...
            t_wa = Catcha.fetch_target_item_by_source(anno.serialized, t.target_source)
            ...
            if t.target_media in [VIDEO, AUDIO]:
                ...
                msg = ("anno({}) is a reply to a reply({}): not " "supported!").format(
                    anno.anno_id, anno.anno_reply_to.anno_id
                )
                logger.error(msg, exc_info=True)
                raise AnnotatorJSError(msg)
                ...
                msg = (
                    "anno({}) media type({}) not supported by " "annotatorjs"
    ...
    def convert_target_single_image(cls, anno, catcha_target_item):
        resp = {"rangePosition": [], "uri": catcha_target_item["source"]}
    ...
    def convert_target_choice_image(cls, anno):
        ...
        for t in anno.raw["target"]["items"]:
            if t["type"] == IMAGE:
                ...
                # TODO: review to return proper exception
                msg = (
                    "anno({}): do not know how to treat target CHOICE for "
                    "target type({})"
        ...
        if i_resp is None:
            # TODO: review to return proper exception
            msg = ("anno({}): expected target media image, but none " "found!").format(
                anno.anno_id
            )
            logger.error(msg, exc_info=True)
            raise AnnotatorJSError(msg)
    ...
    def convert_to_catcha(cls, annojs):
        ...
        try:
            ...
            raise AnnotatorJSError(
                "anno({}): expected property not found - {}".format(anno_id, str(e))
            )
        ...
        if media == "comment":
            ...
            catcha["platform"]["target_source_id"] = catcha["target"]["items"][0][
                "source"
            ]
        elif media == "text":
            ...
            raise AnnotatorJSError(
                "anno({}): unable to process media({})".format(annojs["id"], media)
            )
            ...
            "items": [
                {
                    "type": "TextualBody",
                    "purpose": (
                        "replying" if annojs["media"] == "comment" else "commenting"
                    ),
                    "value": body_text,
                }
            ],
    ...
    def convert_to_catcha_target_image(cls, annojs):
        ...
        for pos in rangePositionList:
            if isinstance(pos, dict):
                ...
                selector["items"].append(cls.strategy_2_1_for_target_selector(annojs))
...
class Catcha(object):
    ...
    def normalize(cls, annotation):
        ...
        if "@context" in annotation:  # assumes webannotation
            try:
                ...
                msg = (
                    "failed to translate input annotation({}) to catcha "
                    "jsonld context: {}"
                ).format(annotation["id"], e)
            ...
            try:
                ...
                msg = (
                    "failed to convert annojs({}) to catcha " "- not annotatorjs?: {}"
                ).format(annotation["id"], e)
                logger.error(msg, exc_info=True)
                raise InvalidInputWebAnnotationError(msg)
    ...
    def check_json_schema(cls, catcha):
        ...
        try:
            ...
            msg = (
                "failed to validate input catcha({}) against catch json " "schema: {}"
            ).format(catcha.get("id", "NA"), e)
            logger.error(msg, exc_info=True)
            raise InvalidInputWebAnnotationError(msg)
    ...
    def safe_body_text_value(cls, catcha):
        ...
        for x in catcha["body"]["items"]:
            for r in CATCH_ANNO_REGEXPS:
                if r.search(x["value"]):
                    msg = (
                        "annotation body contains forbidden chars "
    ...
    def check_for_create_conflicts(cls, catcha, requesting_user):
        ...
        if catcha["creator"]["id"] != requesting_user:
            msg = (
                ...
                "requesting_user({})"
            ).format(catcha["id"], catcha["creator"]["id"], requesting_user)
            logger.error(msg, exc_info=True)
        ...
        if not cls.is_creator_in_permissions(catcha):
            msg = (
                "creator permissions missing in annotation({}): "
                "creator({}) must have permission for at least: "
                "read, write, update"
            ).format(catcha["id"], catcha["creator"]["id"])
            logger.error(msg, exc_info=True)
    ...
    def expand_compact_for_context(cls, annotation, context):
        ...
        try:
            ...
            msg = "compaction for context({}) of anno({}) failed: {}".format(
                original_context, annotation["id"], str(e)
            )
            logger.error(msg, exc_info=True)
    ...
    def are_similar(cls, catcha1, catcha2):
        ...
        for key in ["@context", "id", "type", "schema_version", "creator", "platform"]:
