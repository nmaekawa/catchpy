...
def test_search_by_username_ok(wa_audio):
    ...
    request = make_json_request(
        method="get", query_string="username={}".format(catcha["creator"]["name"])
    )
...
def test_search_by_target_source_ok(wa_audio, wa_image):
    ...
    request = make_json_request(
        method="get", query_string="target_source={}".format(tsource)
    )
    request.catchjwt = payload
...
def test_search_by_body_text_ok(wa_list):
    ...
    payload = make_jwt_payload()
    request = make_json_request(
        method="get", query_string="text={}".format(search_text)
    )
    request.catchjwt = payload
...
def test_search_by_context_id_ok(wa_text, wa_video, wa_image, wa_audio):
    ...
    for z in tudo:
        print(
            "id({}), platform({}), context_id({}), collection_id({})".format(
                z.anno_id,
                z.raw["platform"].get("platform_name", "na"),
...
def test_search_back_compat_by_context_id_ok(wa_list):
    ...
    query_string = "contextId={}&context_id={}&platform={}".format(
        search_context_id,
        "contextId_that_should_be_ignored",
...
def test_search_replies_js_ok(js_list):
    ...
    search_url = (
        "{}?context_id={}&collection_id={}&media=Annotation&" "limit=-1&parentid={}"
    ).format(
    ...
    response = client.get(search_url, HTTP_X_ANNOTATOR_AUTH_TOKEN=token)
...
def test_search_replies_catcha_ok(wa_list):
    ...
    # search for the replies
    search_url = (
        "{}?context_id={}&collection_id={}&media=Annotation&"
        "limit=-1&target_source_id={}"
    ).format(
    ...
    response = client.get(search_url, HTTP_AUTHORIZATION="token {}".format(token))
...
def test_search_deleted_catcha_ok(wa_list):
    ...
    for i in range(1, total_annotations):
        crud_url = reverse("crud_api", kwargs={"anno_id": anno_list[i].anno_id})
    ...
    # search for the replies
    search_url = ("{}?context_id={}&collection_id={}&limit=-1").format(
        reverse("create_or_search"),
...
def test_search_private_catcha_ok(wa_list):
    ...
    response = client.get(
        search_url, HTTP_AUTHORIZATION="token {}".format(token_creator)
    )
    ...
    response = client.get(
        search_url, HTTP_AUTHORIZATION="token {}".format(token_peasant)
    )
    ...
    response = client.get(search_url, HTTP_AUTHORIZATION="token {}".format(token_admin))
...
def test_search_replies_js_with_uri(js_list):
    ...
    # search for the replies
    search_url = (
        "{}?context_id={}&collection_id={}&media=comment&"
        "limit=-1&uri=someFakeId&parentid={}"
    ).format(
        reverse("compat_search"),
        reply_to.raw["platform"]["context_id"],
        reply_to.raw["platform"]["collection_id"],
